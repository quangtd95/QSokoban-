 ...
 * on 10/14/2018.
 */
class Ground(x: Int, y: Int) : Sprite(x, y) {
    ...
    override fun draw(canvas: Canvas, paint: Paint) {
        matrix.reset()
        matrix.setScale(widthCell / imageManager.ground.width, widthCell / imageManager.ground.height)
        matrix.postTranslate(widthCell * x, widthCell * y)
        canvas.drawBitmap(imageManager.ground, matrix, paint)
    }
}